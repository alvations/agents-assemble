...
class DividendInvestor(BasePersona):
    ...
    def generate_signals(self, date, prices, portfolio, data):
        ...
        # Equal weight across all dividend stocks
        all_syms = [sym for sym, _ in candidates]
        if all_syms:
            per_stock = min(0.90 / len(all_syms), self.config.max_position_size)
...
class QuantStrategist(BasePersona):
    ...
    def generate_signals(self, date, prices, portfolio, data):
        ...
        # Vol-weighted sizing
        if candidates:
            total_inv_vol = sum(1 / max(v, 0.005) for _, _, v in candidates)
            for sym, score, vol in candidates[:self.config.max_positions]:
...
class GrowthInvestor(BasePersona):
    ...
    def generate_signals(self, date, prices, portfolio, data):
        ...
        for sym in self.config.universe:
            ...
            if price > sma200 and rsi and 30 < rsi < 55:
