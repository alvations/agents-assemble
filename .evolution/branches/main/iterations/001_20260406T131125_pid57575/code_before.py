...
def fetch_multiple_ohlcv(
    ...
    # Fallback: individually fetch any missing symbols
...
def fetch_yield_curve(date: Optional[str] = None) -> Dict[str, float]:
    ...
    for series_id, label in maturities.items():
        try:
            df = fetch_fred_series(series_id, start="2024-01-01")
            if not df.empty:
                if date:
...
def screen_by_fundamentals(
    ...
    for sym in symbols:
        try:
            ...
            if min_market_cap and (f["market_cap"] or 0) < min_market_cap:
                continue
            if max_pe and f["pe_ratio"] and f["pe_ratio"] > max_pe:
                continue
            if min_dividend_yield and (f["dividend_yield"] or 0) < min_dividend_yield:
                continue
            if max_debt_to_equity and f["debt_to_equity"] and f["debt_to_equity"] > max_debt_to_equity:
                continue
            results.append(f)
        except Exception:
            pass
