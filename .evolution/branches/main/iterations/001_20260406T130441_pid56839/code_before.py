...
def compute_metrics(
    ...
    downside_dev = downside.std() * np.sqrt(periods_per_year) if len(downside) > 0 else 0.001
    ...
    def _rebalance(self, portfolio: Portfolio, target_weights: Dict[str, float],
        ...
        for sym, target_w in target_weights.items():
            ...
            if diff_value > 0:
                ...
                if qty > 0:
                    if current_pos:
                        qty = min(qty, int(abs(current_pos.quantity)))
                    if qty > 0:
...
def format_report(results: Dict[str, Any], title: str = "Backtest Report") -> str:
    ...
    lines = [
        ...
        "",
        "--- Portfolio ---",
        f"  Initial Value:      ${results.get('final_value', 0) - results.get('final_value', 0) + 100000:>10,.2f}",
