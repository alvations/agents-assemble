#!/usr/bin/env python3
"""Backtest 11 fixed strategies on all 28 rolling windows."""
...
STRATEGIES = [
    # 11 fixed strategies
    'benjamin_graham', 'ensemble', 'howard_marks', 'insider_buying_acceleration',
    'insider_buying_real', 'multi_factor_combined', 'prince_alwaleed', 'vix_fear_buy',
    'all_weather_modern', 'fixed_income', 'quant',
    # 10 new strategies
    'covered_call_income', 'dual_momentum_global', 'low_vol_quality',
    'dollar_cycle_rotation', 'managed_futures_proxy', 'preferred_equity_income',
    'infrastructure_reshoring', 'tax_harvest_rotation', 'cross_asset_carry',
    'leveraged_trend_tactical',
]
...
def main():
    ...
    for idx, name in enumerate(STRATEGIES, 1):
        ...
        mw_data[name] = {
            'src': mw_data.get(name, {}).get('src', 'fixed'),
            'desc': mw_data.get(name, {}).get('desc', ''),
            'w': window_results,
